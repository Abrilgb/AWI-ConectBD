  ...
  app.post('/update', async (req, res) => {
    ...
    sessionItem.lastAccessed = now.utc().format(); // Guardar en UTC

    await user.save(); // Mongoose actualizará updatedAt automáticamente

    // Convertir a la zona horaria correcta antes de enviar la respuesta
    ...
    const updatedAtInTime = moment.utc(user.updatedAt).tz('America/Mexico_City').format(); 
    const lastActivityInTime = moment.utc(sessionItem.lastActivity).tz('America/Mexico_City').format();

    const inactividad = now.diff(moment.utc(sessionItem.lastAccessed).tz('America/Mexico_City'), 'minutes');
    const duracion = now.diff(moment.utc(sessionItem.createdAt).tz('America/Mexico_City'), 'minutes');

    res.status(200).json({
      message: 'Datos actualizados',
      session: {
        ...sessionItem.toObject(),
        lastActivity: lastActivityInTime,
        lastAccessed: lastAccessedInTime, // Enviar en zona CDMX
        createdAt: createdAtInTime, // Enviar en zona CDMX
  ...
  app.get('/status', async (req, res) => {  
    ...
    const lastAccessedInTime = moment.utc(sessionItem.lastAccessed).tz('America/Mexico_City').format();  
    const createdAtInTime = moment.utc(sessionItem.createdAt).tz('America/Mexico_City').format(); 
    const updatedAtInTime = moment.utc(user.updatedAt).tz('America/Mexico_City').format(); // Convertir updatedAt 
